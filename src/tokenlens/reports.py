...
def report_sarif(report: AnalysisReport) -> str:
    ...
    payload = {
        ...
        "version": "2.1.0",
        "runs": [{"tool": {"driver": {"name": "TokenLens for Azure", "version": report.version, "rules": [{key: value for key, value in rule.items() if key != "confidence"} for rule in report.rules]}}, "results": results}],
    }
...
def report_html(report: AnalysisReport) -> str:
    ...
    for finding in report.findings:
        ...
        if estimate.unit == "calls":
            ...
            impact = "Impact requires quality validation"
        else:
            impact = f"{estimate.min_tokens:,}–{estimate.max_tokens:,} tokens"
        if finding.impact_min_percent is not None:
            if finding.impact_min_percent == finding.impact_max_percent:
                impact_percent = f"{finding.impact_min_percent:.1f}% impact"
            else:
                impact_percent = (
                    f"{finding.impact_min_percent:.1f}–"
                    f"{finding.impact_max_percent:.1f}% impact"
                )
        else:
        ...
        finding_rows.append(
            f"""<article class="finding {html.escape(_severity_class(finding))}">
              ...
              <div class="impact">{html.escape(impact_percent)}<small>{html.escape(impact)}</small></div>
