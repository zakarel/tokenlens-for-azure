...
from .pricing_sources.assumptions import ASSUMPTIONS_BANNER, ASSUMPTIONS_WARNING, USD_PER_CCU
...
def _pricing_provenance(report: AnalysisReport, models) -> str:
    ...
    details: list[str] = [
        f"<li><strong>{_escape(str(pricing.get('assumptions_banner') or ASSUMPTIONS_BANNER))}</strong></li>",
        f"<li>{_escape(str(pricing.get('assumptions_warning') or ASSUMPTIONS_WARNING))}</li>",
    ...
    applied = pricing.get("assumed_dimensions_applied") or []
    if applied:
        details.append(
            "<li>Defaults actually applied to a matched rate: "
            f"{_escape(', '.join(str(item) for item in applied))}</li>"
        )
    for source in pricing.get("public_sources") or []:
        if not isinstance(source, dict):
            continue
        details.append(
            f"<li>Synchronized source: {_escape(str(source.get('source')))} · retrieved "
            f"{_escape(str(source.get('retrieved_at')))} · {_escape(str(source.get('content_hash')))} · "
            f"{_escape(str(source.get('entries')))} entry(ies), "
            f"{_escape(str(source.get('quarantined')))} quarantined</li>"
        )
    if matched:
    ...
    claude_models = [item for item in models if (item.pricing_publisher or "").casefold() == "anthropic"]
    if claude_models:
        claude_cost = sum(item.estimated_cost_usd or 0.0 for item in claude_models)
        equivalent = (
            f": estimated {_money(claude_cost, currency=summary.pricing_currency)} "
            f"≈ {claude_cost / USD_PER_CCU:,.0f} CCU"
            if claude_cost
            else ""
        )
        details.append(
            "<li>Claude-family billing basis is a CCU-derived dollar-equivalent estimate at "
            f"{USD_PER_CCU:g} USD per consumption unit (100 CCU = $1), not an Azure token meter"
            f"{equivalent}. Customer-specific private discounts are not included.</li>"
        )
    details.append(
...
def _pricing_assumptions_banner(report: AnalysisReport) -> str:
    """The bold assumptions line and its override warning, above everything else.

    These defaults are only ever applied where telemetry or deployment metadata
    did not state the dimension, so they are shown as an explicit assumption
    rather than folded silently into a number.
    """
    pricing = report.report_metadata.get("pricing", {})
    banner = str(pricing.get("assumptions_banner") or ASSUMPTIONS_BANNER)
    warning = str(pricing.get("assumptions_warning") or ASSUMPTIONS_WARNING)
    applied = pricing.get("assumed_dimensions_applied") or []
    applied_line = (
        f'<span class="assumption-applied">Applied to matched rates: '
        f'{_escape(", ".join(str(item) for item in applied))}</span>'
        if applied
        else ""
    )
    return (
        f'<section class="pricing-assumptions" role="note" aria-label="Pricing assumptions" '
        f'title="{_escape(warning)}">'
        f'<p class="assumption-banner"><strong>{_escape(banner)}</strong></p>'
        f'<p class="assumption-warning"><span class="assumption-icon" aria-hidden="true">!</span>'
        f'{_escape(warning)}</p>{applied_line}</section>'
    )
...
def _cost_analysis_panel(report: AnalysisReport) -> str:
    ...
    return f"""{_pricing_assumptions_banner(report)}
    <section class="state-banner state-banner-{banner.tone}">{_availability_badge(banner)}{action_link}</section>
    {identity_banner}
